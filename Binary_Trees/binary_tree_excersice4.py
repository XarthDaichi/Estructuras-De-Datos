...
class Binary_Tree_EX4:
    ...
    # Dado un arbol binario, cree un algoritmo que dado un nodo X, retorne todos sus ancestros
    def ancestors(self, searching:Node):
        return self._ancestors(searching, self.root)

    def _ancestors(self, searching:Node, current:Node):
        if current is None:
            return

        if current is searching:
            return []

        ancestors = None
        left = self._ancestors(searching, current.left)
        if left is not None:
            ancestors = left + [current.value]
        right = self._ancestors(searching, current.right)
        if right is not None:
            ancestors = right + [current.value]
        return ancestors
