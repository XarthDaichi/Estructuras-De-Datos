...
public:
	...
	listptr invert_list(listptr node, listptr original_start, listptr before_node = nullptr) {
		...
		listptr new_before = invert_list(node->next, start, node);
		if (before_node) before_node->next = nullptr;
		if (node != original_start) node->next = original_start;
