...
public:
	...
	void insert_in_pos(const T& val, std::size_t pos) {
		listptr new_node;
		try {
			new_node = new Node(val);
		}
		...
		if (start == nullptr) {
			...
			if (pos == 0) {
				...
				listptr tmp = start;
				for (std::size_t i = 1; i < pos && tmp->next != nullptr; i++) {
					tmp = tmp->next;
				}
				listptr tmp_next = tmp->next;
				listptr tmp_before = tmp->before;
				tmp->next = new_node;
				new_node->before = tmp_before;
